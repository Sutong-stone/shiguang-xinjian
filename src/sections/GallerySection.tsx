...
export default function GallerySection() {
  ...
  const fullText =
    "今天是你满月的日子，看着你熟睡的小脸，觉得整个世界都安静了下来...";
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (index <= fullText.length) {
        setDialogText(fullText.slice(0, index));
        index++;
      } else {
        clearInterval(timer);
      }
  ...
  const column1 = albums?.filter((_, i) => i % 3 === 0) || [];
  const column2 = albums?.filter((_, i) => i % 3 === 1) || [];
  const column3 = albums?.filter((_, i) => i % 3 === 2) || [];
    ...
    >
      ...
      >
        <div className="relative bg-white rounded-3xl rounded-br-md px-6 py-5 max-w-md shadow-soft">
          <p className="text-[#8C7B72] text-sm leading-relaxed min-h-[3rem]">
            {dialogText}
            <span className="animate-pulse-soft inline-block w-0.5 h-4 bg-[#D4A78C] ml-1 align-middle" />
          </p>
      ...
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        >
          ...
          <p className="text-sm text-[#D4A78C]">
            每一张照片，都是时光的标本
          </p>
        </motion.div>
        ...
        >
          {columns.map((col, colIdx) => (
            <div
              key={colIdx}
              className="space-y-6"
              style={{ transform: `translateY(${colIdx * 40}px)` }}
            >
              {col.map((album, idx) => (
                <GalleryCard
                  key={album.id}
